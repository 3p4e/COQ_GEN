/* ============================================================================
   Planner Core API client. Base http://127.0.0.1:8765.
   Auth: JWT bearer (per-user login), unlike the desktop app's X-COQGEN-Token.
   The token is held in memory + mirrored to localStorage so a refresh keeps the
   session. 401 responses clear it and bubble up so the shell can show the login.
   ============================================================================ */

const BASE =
  (import.meta as { env?: Record<string, string> }).env?.VITE_PLANNER_API ?? "http://127.0.0.1:8765";
const STORAGE_KEY = "coqgen_planner_token";
