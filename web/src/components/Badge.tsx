...
export function Badge({ status, label, size = "md", dot = true }: BadgeProps) {
  ...
  const style: CSSProperties = {
    ...
    padding: sm ? "1px 7px" : "2px 9px", borderRadius: "var(--radius-full)",
    fontSize: sm ? 10 : 11, fontWeight: 600, lineHeight: 1.5,
    background: tone.bg, color: tone.fg, border: `1px solid ${tone.border}`,
    fontFamily: "var(--font-sans)", whiteSpace: "nowrap", textTransform: "capitalize",
  };
  return (
    <span style={style}>
      {dot && <span style={{ width: sm ? 5 : 6, height: sm ? 5 : 6, borderRadius: "50%", background: tone.dot, flexShrink: 0 }} />}
      {label ?? tone.label}
    </span>
  );
}
